...
export class CreateComponent extends BaseDetachedComponent implements OnInit {
    ...
    stepIsValid(value: ESteps): boolean {
        switch (value) {
            case ESteps.general:
                return ['name','code','category','status','actions'].filter((key: string) => {
    ...
    private build(): void {
        this.form = this.fb.group({
            ...
            category: [null, [Validators.required, Validators.maxLength(255), WhitespaceValidator]],
            status: [null, [Validators.required, Validators.maxLength(255), WhitespaceValidator]],
            actions: [null, [Validators.required, Validators.maxLength(255), WhitespaceValidator]]
